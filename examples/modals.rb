...
bot = OnyxCord::Bot.new(token: ENV.fetch('ONYXCORD_TOKEN'))
bot.register_application_command(:modal_test, 'Test out a spiffy modal', server_id: ENV.fetch('ONYXCORD_SERVER_ID'))
bot.register_application_command(:modal_await_test, 'Test out the await style', server_id: ENV.fetch('ONYXCORD_SERVER_ID'))
...
bot.application_command :modal_await_test do |event|
  ...
  modal_event = bot.add_await!(OnyxCord::Events::ModalSubmitEvent, custom_id: id)

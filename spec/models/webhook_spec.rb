...
describe OnyxCord::Webhook do
  ...
  describe '#avatar_url' do
    context 'avatar is set' do
      it 'calls the correct API helper' do
        expect(OnyxCord::REST::User).to receive(:avatar_url).with(webhook_id, webhook_avatar)
        webhook.avatar_url
      end
    end
    ...
    context 'avatar is not set' do
      ...
      it 'calls the correct API helper' do
        expect(OnyxCord::REST::User).to receive(:default_avatar)
        webhook.avatar_url
      end
    end
  end
  ...
  describe '#execute' do
    let(:resp) { instance_double(OnyxCord::Internal::HTTP::Response) }
    let(:data) { instance_double(Hash) }

    before do
      allow(OnyxCord::REST::Webhook).to receive(:token_execute_webhook)
    end
    ...
    it 'merges kwargs with builder data' do
      ...
      expect(OnyxCord::REST::Webhook).to have_received(:token_execute_webhook).with(anything, anything, false, content, username, any_args)
    end

    context 'when wait is true' do
      before do
        allow(OnyxCord::REST::Webhook).to receive(:token_execute_webhook).and_return(resp)
  ...
  describe '#edit_message' do
    ...
    before do
      allow(OnyxCord::REST::Webhook).to receive(:token_edit_message).with(any_args).and_return(resp)
    ...
    it 'merges kwargs with builder data' do
      ...
      expect(OnyxCord::REST::Webhook).to have_received(:token_edit_message).with(webhook.token, webhook.id, message_id, content, embeds, nil, [], nil, 0)
    end
  ...
  describe '#delete_message' do
    ...
    before do
      allow(OnyxCord::REST::Webhook).to receive(:token_delete_message).with(any_args)
    end
    ...
    it 'calls token_delete_message' do
      ...
      expect(OnyxCord::REST::Webhook).to have_received(:token_delete_message).with(webhook.token, webhook.id, message_id)
    end
  end
end

...
describe OnyxCord::Message do
  ...
  describe '#emoji' do
    it 'caches and returns only emojis from the message' do
      emoji_a = OnyxCord::Emoji.new({ 'name' => 'a', 'id' => 123 }, bot, server)
      emoji_b = OnyxCord::Emoji.new({ 'name' => 'b', 'id' => 456 }, bot, server)
  ...
  describe '#reacted_with' do
    ...
    before do
      ...
      allow(OnyxCord::REST::Channel).to receive(:get_reactions)
    ...
    it 'calls the API method' do
      expect(OnyxCord::REST::Channel).to receive(:get_reactions)
    ...
    it 'fetches all users when limit is nil' do
      expect(OnyxCord::Paginator).to receive(:new).with(nil, :down)
  ...
  describe '#reply!' do
    ...
    context 'when allowed_mentions is an AllowedMentions' do
      ...
      let(:allowed_mentions) { instance_double('OnyxCord::AllowedMentions', 'allowed_mentions') }
  ...
  describe '#respond' do
    ...
    let(:embed) { instance_double('OnyxCord::Webhooks::Embed', 'embed') }
    let(:attachments) { instance_double('Array', 'attachments') }
    let(:allowed_mentions) { instance_double('Hash', 'allowed_mentions') }
    let(:message_reference) { instance_double('OnyxCord::Message') }
    let(:components) { instance_double('OnyxCord::Webhooks::View') }

...
describe OnyxCord::Permissions do
  ...
  describe OnyxCord::Permissions::FLAGS do
    it 'creates a setter for each flag' do
      responds_to_methods = OnyxCord::Permissions::FLAGS.map do |_, flag|
    ...
    it 'calls #write on its writer' do
      ...
      OnyxCord::Permissions.new(0, writer).can_read_messages = true
    end
  end

  context 'with FLAGS stubbed' do
    before do
      stub_const('OnyxCord::Permissions::FLAGS', 0 => :foo, 1 => :bar)
    end
    ...
    describe '.bits' do
      it 'returns the correct packed bits from an array of symbols' do
        expect(OnyxCord::Permissions.bits(%i[foo bar])).to eq 3
      end
    end
    ...
    describe '#initialize' do
      ...
      it 'can initialize with an array of symbols' do
        instance = OnyxCord::Permissions.new %i[foo bar]
        expect(instance.bits).to eq 3
      end

      it 'calls #init_vars' do
        expect_any_instance_of(OnyxCord::Permissions).to receive(:init_vars)
        subject
      end
    end
    ...
    describe '#defined_permissions' do
      it 'returns the defined permissions' do
        instance = OnyxCord::Permissions.new 3
...
describe OnyxCord::PermissionCalculator do
  ...
  describe '#defined_role_permission?' do
    it 'solves permissions (issue #607)' do
      everyone_role = double('everyone role', id: 0, position: 0, permissions: OnyxCord::Permissions.new)
      role_a = double('role a', id: 1, position: 1, permissions: OnyxCord::Permissions.new)
      role_b = double('role b', id: 2, position: 2, permissions: OnyxCord::Permissions.new([:manage_messages]))
    ...
    it 'takes overwrites into account' do
      everyone_role = double('everyone role', id: 0, position: 0, permissions: OnyxCord::Permissions.new)
      role_a = double('role a', id: 1, position: 1, permissions: OnyxCord::Permissions.new([:manage_messages]))
      role_b = double('role b', id: 2, position: 2, permissions: OnyxCord::Permissions.new)
      channel = double('channel')

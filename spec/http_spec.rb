...
describe OnyxCord::Internal::HTTP do
  ...
  it 'shares one persistent session across threads' do
    described_class.reset!
    ...
    expect(worker_session).to be(main_session)
  end

  it 'forces REST requests over HTTP/1.1 with a bounded pool' do
    class_double(HTTPX)
  ...
  it 'closes the current session on reset' do
    ...
    expect(described_class.session).not_to be(session)
  end

...
describe OnyxCord::Channel do
  ...
  describe '#update_channel_data' do
    shared_examples('API call') do |property_name|
      it "should call the API with #{property_name}" do
        ...
        expect(OnyxCord::API::Channel).to receive(:update!)
    ...
    it 'should call #update_data with new data' do
      ...
      allow(OnyxCord::API::Channel).to receive(:update!)
      channel.__send__(:modify)
    end

    context 'when NoPermission is raised' do
      it 'should not call update_data' do
        allow(OnyxCord::API::Channel).to receive(:update!).and_raise(OnyxCord::Errors::NoPermission)
  ...
  describe '#update_data' do
    ...
    it 'should call process_permission_overwrites' do
      allow(OnyxCord::API::Channel).to receive(:resolve).and_return('{}')
    ...
    context 'when data is not provided' do
      it 'should request it from the API' do
        expect(OnyxCord::API::Channel).to receive(:resolve).and_return('{}')
        channel.__send__(:update_data)
      end
    end
  end
  ...
  describe '#bulk_delete' do
    it 'should log with old messages' do
      ...
      allow(OnyxCord::API::Channel).to receive(:bulk_delete_messages)
  ...
  describe '#process_permission_overwrites' do
    it 'should assign permission overwrites' do
      ...
      element = { 'id' => 1 }
      overwrites = [element]
      allow(OnyxCord::Overwrite).to receive(:from_hash).and_call_original
      allow(OnyxCord::Overwrite).to receive(:from_hash).with(element).and_return(overwrite)
  ...
  describe '#sort_after' do
    it 'should call the API' do
      ...
      expect(OnyxCord::API::Server).to receive(:update_channel_positions)

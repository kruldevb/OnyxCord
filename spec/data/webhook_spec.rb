...
describe OnyxCord::Webhook do
  ...
  describe '#delete' do
    context 'when webhook is from auth' do
      it 'calls the API' do
        expect(OnyxCord::API::Webhook).to receive(:delete_webhook).with(token, webhook_id, reason)
    ...
    context 'when webhook is from token' do
      ...
      it 'calls the token API' do
        expect(OnyxCord::API::Webhook).to receive(:token_delete_webhook).with(webhook_token, webhook_id, reason)
        webhook.delete(reason)
      end
    end
  end
  ...
  describe '#avatar_url' do
    context 'avatar is set' do
      it 'calls the correct API helper' do
        expect(OnyxCord::API::User).to receive(:avatar_url).with(webhook_id, webhook_avatar)
        webhook.avatar_url
      end
    end
    ...
    context 'avatar is not set' do
      ...
      it 'calls the correct API helper' do
        expect(OnyxCord::API::User).to receive(:default_avatar)
        webhook.avatar_url
      end
    end
  end
  ...
  describe '#update_webhook' do
    context 'API returns valid data' do
      it 'calls update_internal' do
        ...
        allow(OnyxCord::API::Webhook).to receive(:update_webhook)
    ...
    context 'when webhook is from auth' do
      it 'calls auth API' do
        ...
        expect(OnyxCord::API::Webhook).to receive(:update_webhook).with(token, webhook_id, data, reason)
    ...
    context 'when webhook is from token' do
      ...
      it 'calls token API' do
        data = double('data', delete: reason)
        allow(JSON).to receive(:parse).and_return(double('received_data', :[] => double))
        expect(OnyxCord::API::Webhook).to receive(:token_update_webhook).with(webhook_token, webhook_id, data, reason)
        webhook.send(:update_webhook, data)
      end
    end
  end
  ...
  describe '#execute' do
    ...
    before do
      allow(OnyxCord::API::Webhook).to receive(:token_execute_webhook)
    end
    ...
    context 'when there is no token' do
      ...
      it 'raises an UnauthorizedWebhook error' do
        expect { tokenless_webhook.execute }.to raise_error(OnyxCord::Errors::UnauthorizedWebhook)
      end
    end

    context 'when no builder is provided' do
      it 'creates a new builder' do
        expect { |b| webhook.execute(wait: false, &b) }.to yield_with_args(instance_of(OnyxCord::Webhooks::Builder), instance_of(OnyxCord::Webhooks::View))
      end
    end
    ...
    it 'merges kwargs with builder data' do
      ...
      builder = OnyxCord::Webhooks::Builder.new(content: content)

      webhook.execute(username: username, builder: builder, wait: false)

      expect(OnyxCord::API::Webhook).to have_received(:token_execute_webhook).with(anything, anything, false, content, username, any_args)
    end

    context 'when wait is true' do
      before do
        allow(OnyxCord::API::Webhook).to receive(:token_execute_webhook).and_return(resp)
      ...
      it 'creates a Message object with the response' do
        ...
        expect(OnyxCord::Message).to have_received(:new).with(data, bot)
      end
    end
  end
  ...
  describe '#edit_message' do
    ...
    before do
      allow(OnyxCord::API::Webhook).to receive(:token_edit_message).with(any_args).and_return(resp)
      allow(JSON).to receive(:parse).with(anything).and_call_original
      allow(JSON).to receive(:parse).with(resp).and_return(data)
      allow(OnyxCord::Message).to receive(:new).with(any_args).and_return(nil)
    end
    ...
    context 'when there is no token' do
      ...
      it 'raises an UnauthorizedWebhook error' do
        expect { tokenless_webhook.edit_message(message) }.to raise_error(OnyxCord::Errors::UnauthorizedWebhook)
      end
    end
    ...
    it 'merges kwargs with builder data' do
      ...
      expect(OnyxCord::API::Webhook).to have_received(:token_edit_message).with(webhook.token, webhook.id, message_id, content, embeds, nil, [], nil, 0)
    end

    it 'returns an updated Message object' do
      msg = instance_double(OnyxCord::Message)
      allow(OnyxCord::Message).to receive(:new).with(data, bot).and_return(msg)
  ...
  describe '#delete_message' do
    let(:message) { instance_double(OnyxCord::Message, resolve_id: message_id) }
    let(:message_id) { instance_double(Integer) }

    before do
      allow(OnyxCord::API::Webhook).to receive(:token_delete_message).with(any_args)
    end
    ...
    context 'when there is no token' do
      ...
      it 'raises an UnauthorizedWebhook error' do
        expect { tokenless_webhook.delete_message(message_id) }.to raise_error(OnyxCord::Errors::UnauthorizedWebhook)
      end
    end
    ...
    it 'calls token_delete_message' do
      ...
      expect(OnyxCord::API::Webhook).to have_received(:token_delete_message).with(webhook.token, webhook.id, message_id)
    end
  end
end
